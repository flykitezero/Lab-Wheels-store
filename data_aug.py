...
提交人：吴彪
提交日期：2021/7/22
函数作用：网络的输入部分进行数据增强：包括随机水平翻转，中心裁剪，调整亮度，对比度，归一化处理
特殊导入包： from torvision import transforms
使用方法：首先定义transform变量，使用方法aug=transform（Input）
          注意：输入必须是PIL文件；
          得到的输出为归一化了的tensor类型数据
'''
...
transform=transforms.Compose([
                                                                                Scale_ada_union([224, 304]),
                                                                                RandomHorizontalFlip_ada_union(),
                                                                                CenterCrop_ada_union([304, 224], [152, 112]),     #[304, 224] is image size read by Image.size
                                                                                                                            #[152, 112] is depth size read by Image.size
                                                                                                                            #Image.size is Width and Height
                                                                                ToTensor_ada_union(),
                                                                                Lighting_ada_union(0.1, __imagenet_pca['eigval'], __imagenet_pca['eigvec']),
                                                                                ColorJitter_ada_union(
                                                                                                brightness=0.4,
                                                                                                contrast=0.4,
                                                                                                saturation=0.4,
                                                                                                ),
                                                                                Normalize_ada_union(__imagenet_stats['mean'], __imagenet_stats['std'])
                                                                                ])
                                                    )
#定义相关的数据增强操作
...
class RandomRotate(object):
    ...
    def __call__(self, sample):
        image, depth = sample['colored_image'], sample['depth']
        ...
        return {'colored_image': image, 'depth': depth}
...
# depth's value will be multiply 10
# for testing
